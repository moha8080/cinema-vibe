...
export default function TvHub() {
  ...
  return (
    <div dir="rtl" style={{ backgroundColor: '#09090b', color: '#f4f4f5', minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      <Head>
        <title>قسـم المسلسلات | سينما فايب</title>
      </Head>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {GENRES.map(g => (
              <button 
                ...
                style={{ padding: '10px 12px', textAlign: 'right', backgroundColor: hubGenre === g.id ? '#f97316' : 'transparent', color: hubGenre === g.id ? '#000' : '#d4d4d8', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: hubGenre === g.id ? 'bold' : 'normal' }}
              >
                {g.name}
              </button>
            ))}
